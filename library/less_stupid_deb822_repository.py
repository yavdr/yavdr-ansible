...
options:
    ...
    mangle_name:
        description:
        - Mangle the name. Ansible by default replaces all whitespace and underscores with a single dash.
          Then it converts the string to lower case and removes any characters that are not alphanumeric or a dash.
          If set to false, no name mangling is applied and the name must only contain characters matching this regex: '^[a-zA-Z0-9_.-]+$' -
          see 'man sources.list' for details.
          required: false
          type: bool
    pdiffs:
        description:
...
def install_python_debian(module, deb_pkg_name):
    if not module.check_mode:
        ...
        if apt_path:
            ...
            if rc != 0:
                module.fail_json(
                    msg=f"Failed update while auto installing {deb_pkg_name} due to '{se.strip()}'"
                )
            rc, so, se = module.run_command(
                [apt_path, "install", deb_pkg_name, "-y", "-q"]
            )
            if rc != 0:
                module.fail_json(
                    msg=f"Failed to auto-install {deb_pkg_name} due to : '{se.strip()}'"
                )
    else:
...
def main():
    ...
    if not HAS_DEBIAN:
        ...
        if has_respawned():
            ...
            module.fail_json(
                msg=f"{deb_pkg_name} must be installed and visible from {sys.executable}."
            )
        ...
        if module.check_mode:
            module.fail_json(
                msg=f"{deb_pkg_name} must be installed to use check mode. If run with install_python_debian, this module can auto-install it."
            )
        ...
        if module.params["install_python_debian"]:
            ...
            module.fail_json(
                msg=f"{deb_pkg_name} is not installed, and install_python_debian is False"
            )
        ...
        if interpreter:
            ...
            module.fail_json(
                msg=missing_required_lib(deb_pkg_name), exception=DEBIAN_IMP_ERR
            )
    ...
    mangle_name = params.pop("mangle_name")
    module.warn(f"debug: {mangle_name=}: {type(mangle_name)=}")
    ...
    if not re.match(r"^[a-zA-Z0-9_.-]+$", name):
        module.fail_json(
            msg="Invalid characters in name '%s' - only [a-zA-Z0-9._-] are allowed - see man sources.list"
            % name,
            rc=1,
        )
