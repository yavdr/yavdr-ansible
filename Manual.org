...
#+BEGIN_SRC shell :tangle install-yavdr-rpi.sh :shebang "#!/bin/bash"
set -e
...
#+BEGIN_SRC yaml :tangle roles/yavdr-common/tasks/configure_system.yml :mkdirp yes
- name: check if bash is the default sh shell
  stat:
    path: /usr/bin/sh
...
- name: use bash instead of dash
  ...
  when: default_shell.stat.lnk_target != '/usr/bin/bash'
#+END_SRC
...
*** MOTD
**** Header
#+begin_src shell :tangle roles/yavdr-common/templates/update-motd.d/00-header.j2 :mkdirp yes :padline no
#!/bin/sh
#
...
#+begin_src jinja2 :tangle roles/vdr/files/reccmds.removemarks.conf.j2
{{ ansible_managed | comment }}
...
This is a snippet
#+begin_src jinja2 :tangle roles/vdr/files/reccmds.searchrepeats.conf.j2
...
#+begin_src shell :tangle roles/vdr/files/reccmds/removeresume.sh :shebang #!/usr/bin/sh
...
#+begin_src shell :tangle roles/vdr/files/reccmds/removeindex.sh :shebang #!/usr/bin/sh
...
#+begin_src shell :tangle roles/vdr/files/reccmds/removemarks.sh :shebang #!/usr/bin/sh
...
#+begin_src shell :tangle roles/vdr/files/reccmds/vdr-recrepeat.sh :shebang #!/usr/bin/bash
#------------------------------------------------------------------------------
...
*** templates
**** config.yml
#+BEGIN_SRC shell :tangle roles/udiskie/templates/config.yml.j2 :mkdirp yes :shebang #!/bin/bash
{{ ansible_managed | comment }}
...
#+END_SRC
**** mount helper script
#+BEGIN_SRC shell :tangle roles/udiskie/templates/udiskie_vdr_mount_helper.j2 :mkdirp yes :shebang #!/bin/bash
{{ ansible_managed | comment }}
...
#+BEGIN_SRC shell :tangle roles/udiskie/templates/umount_all.j2 :mkdirp yes :shebang #!/bin/bash
{{ ansible_managed | comment }}
...
dtoverlay=gpio-ir-recv,gpio_pin={{ rpi_ir_gpio_pin }}
{% endif %}
{% if rpi_ir_gpio_pin_tx is defined %}
dtoverlay=gpio-ir-recv-tx,gpio_pin={{ rpi_ir_gpio_pin_tx }}
{% endif %}
#+END_SRC
** channelpedia upload
