...
#+BEGIN_SRC shell :tangle install-yavdr-rpi.sh :shebang "#!/usr/bin/bash"
set -e
...
#+begin_src yaml :tangle roles/vdr/tasks/main.yml :mkdirp yes
- name: configure dbus2vdr for vdr instance id "{{ vdr.instance_id }}" and user "{{ vdr.user }}"
  vars:
    vdr_instance_id: "{{ '' if vdr.instance_id | int | default(0) == 0 else vdr.instance_id }}"
  template:
...
Environment=VDR_ID={{ vdr.instance_id  }}
#+END_SRC
...
- name: Firmware for Hauppauge Win-TV HVR-4000 and NOVA-HD-S2
  get_url:
    url: https://github.com/OpenELEC/dvb-firmware/blob/master/firmware/dvb-fe-cx24116.fw?raw=true
    checksum: sha256:8fa49be12cf332b4c9b0379ef997be9ab6e193ae03aae55b39e033ae31e35da0
    dest: /lib/firmware/dvb-fe-cx24116.fw
  when: '"14f1:8802" in pci'
  notify: ['reboot required']
