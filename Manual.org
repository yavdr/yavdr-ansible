...
- [[#playbooks][Playbooks]]
  ...
  - [[#yavdr07-arm64yml][yavdr07-arm64.yml]]
- [[#hosts][Hosts]]
...
#+BEGIN_SRC shell :tangle install-yavdr-arm64.sh :shebang "#!/usr/bin/env bash"
set -e
...
#+END_SRC
** yavdr07-arm64.yml
VDR-only setup for Raspberry Pi 4 and 5 on 64 bit installation.
Please note that kodi is not available at the moment.
#+begin_src yaml :tangle yavdr07-arm64.yml :padline no
---
...
- name: set up a headless yaVDR server
  ...
  roles:
    ...
    - collect-facts              # query system facts
    - vdr
    - arm64-output               # install appropriate output plugin and let vdr run on tty7
    - lifeguardng                # install and configure vdr-addon-lifeguard-ng
    - headless-session           # set up a headless systemd user session, e.g. for udiskie
    - yavdr-remote
    - yavdr-network
...
# set the name of the output plugin (as used by vdrctl) - this defaults to softhddevice on x64 platforms
# selected_frontend: softhddevice

# set the package name of the output plugin - this defaults to vdr-plugin-softhddevice on x64 platforms
...
** arm64-output
This role installs vdr-plugin-softhdevice-drm-gles and configures the output to use ~tty7~. A systemd user session allows the use of udiskie and other programs within a systemd user session. Please note that =kodi= is no supported at the moment due to limitation with the dri master.
#+BEGIN_SRC yaml :tangle roles/arm64-output/tasks/main.yml :mkdirp yes :padline no
---
...
#+begin_src jinja2 :tangle roles/arm64-output/templates/systemd/console-output.conf.j2
{{ ansible_managed | comment }}
[Unit]
Conflicts=getty@tty7.service
