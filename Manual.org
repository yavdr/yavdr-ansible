...
#+LATEX_HEADER: \usepackage[locale=DE,separate-uncertainty,repeatunits=true,trapambigerr=false,tophrase={{ bis }}]{siunitx}
#+LATEX_HEADER: \usepackage[breaklines,breakanywhere]{minted}
...
- name: set up yaVDR
  ...
  roles:
    ...
    - osd2web                     # set up osd2web and view it in a browser if a second display is detected
    - autoinstall-satip          # install vdr-plugin-satip if a Sat>IP server has been found
    - autoinstall-targavfd       # install vdr-plugin-targavfd if display is connected
    - autoinstall-imonlcd        # install vdr-plugin-imonlcd if a matchind display is connected
...
#+BEGIN_SRC jinja2 :tangle roles/template-test/templates/xorg.conf_test.j2
{{ ansible_managed | comment }}
...
#+BEGIN_SRC jinja2 :tangle roles/yavdr-xorg/templates/10-serverflags.conf.j2
{{ ansible_managed | comment }}
Section "ServerFlags"
    Option         "NoPM" "true"
    Option         "blank time" "0"
...
enabled_services=(
    ...
    "openbox-second.service" {% if "osd2web" in ansible_role_names %}"osd2web.service"{% endif %}
    "yavdr-frontend.service" "pulseaudio.socket"
    "pulseaudio.service" "wm-exit.service" "irexec.service")
disabled_services=(
    {% if "osd2web" not in ansible_role_names %}"osd2web.service"{% endif %}
)
