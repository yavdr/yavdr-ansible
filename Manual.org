...
<menus>
    ...
    <menu name="{{ 'System' | translate }}">
         ...
         <!--<menu name="{{ 'Commands' | translate }}">
               <command name="{{ "Safely remove usb mass storage" | translate }}" confirm="yes" execute='echo &apos;svdrpsend mesg "$(udiskie-umount -a 2>&amp;1 | grep -o "Error unmounting.*")"&apos; | at now' />
               <command name="{{ "Update vdr recordings list" | translate }}" execute="/usr/bin/vdr-dbus-send /Recordings recording.Update &amp;> /dev/null " />
               <command name="{{ "Restart VDR" |translate }}" confirm="yes" execute="sudo /bin/systemctl --no-block restart vdr.service" />
               <command name="{{ "Reboot system"| translate }}" confirm="yes" execute="sudo /bin/systemctl --no-block reboot" />
