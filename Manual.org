...
[local]
localhost ansible_connection=local ansible_python_interpreter=/root/.ansible-venv/bin/python3
#+END_SRC
* Group Variables
** PPAs
...
*** templates
**** Systemd Service
#+begin_src systemd :tangle roles/headless-session/systemd/headless-session@.service.j2 :mkdirp yes :padline no
[Unit]
...
#+end_src
**** Configuration file
#+begin_src shell :tangle roles/headless-session/config/headless-session
udiskie &
#+end_src
** udiskie
...
#+BEGIN_SRC shell :tangle roles/yavdr-desktop/templates/umount_all.j2 :mkdirp yes :shebang #!/bin/bash
{{ ansible_managed | comment }}
...
** yavdr-desktop
*** Dependencies
#+begin_src yaml :tangle roles/yavdr-desktop/meta/main.yml :mkdirp yes
---
dependencies:
  - role: udiskie
#+end_src
