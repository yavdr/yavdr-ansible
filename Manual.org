...
- name: use bash instead of dash
  ...
  when: default_shell.stat.lnk_source | default("") != '/usr/bin/bash'
#+END_SRC
...
- name: vdr-config | start softhddevice detached and set audio to pulseaudio
  ...
  loop:
      ...
      - '{{ "-v va-vapi" if (intel_detected and not nvidia_detected) else ("-v vdpau" if (nvidia_detected or amd_detected) else "# -v none") }}'
...
#+END_SRC
*** templates
#+BEGIN_SRC jinja2 :tangle roles/grub-config/templates/50_custom.j2 :mkdirp yes :padline no :shebang #!/usr/bin/sh
{{ ansible_managed | comment }}
